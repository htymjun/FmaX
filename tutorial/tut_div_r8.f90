! tut_div_r8.f90  —  fltflt division with real(8) input
!
! Build:  cd tutorial && cmake -B build && cmake --build build -j
! Run:    cd build && ./tut_div_r8
!
...
contains
  ...
  attributes(global) subroutine kern_div_r8(a_in, b_in, res_r4, res_hi, res_lo)
  ...
  end subroutine kern_div_r8

end module tut_div_r8_kern
...
program tut_div_r8
  ...
  call kern_div_r8<<<1,1>>>(A, B, res_r4, res_hi, res_lo)
  ...
  write(*,*) "=== fltflt division (real(8) input): 1.0d0 / 3.0d0 ==="
  write(*,*)
