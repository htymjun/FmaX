! tut_mul.f90  —  fltflt multiplication
!
! Build:  cd tutorial && cmake -B build && cmake --build build -j
! Run:    cd build && ./tut_mul
!
! Demonstrates ff*ff multiplication via FMA-based TwoProd.
! Input a=1.1, b=1.1: exact product 1.21 needs >24 mantissa bits.
! real(4) rounds at 24 bits; fltflt captures the exact product using
! r%lo = fma(a, b, -r%hi) — a single FMAF.F32 instruction on the GPU.
...
program tut_mul
  ...
  real(4), parameter :: A = 1.1_4, B = 1.1_4
  ...
  write(*,*) "=== fltflt multiplication: 1.1 * 1.1 ==="
  write(*,*)
