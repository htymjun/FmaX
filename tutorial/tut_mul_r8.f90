! tut_mul_r8.f90  —  fltflt multiplication with real(8) input
!
! Build:  cd tutorial && cmake -B build && cmake --build build -j
! Run:    cd build && ./tut_mul_r8
!
...
contains
  ...
  attributes(global) subroutine kern_mul_r8(a_in, b_in, res_r4, res_hi, res_lo)
  ...
  end subroutine kern_mul_r8

end module tut_mul_r8_kern
...
program tut_mul_r8
  ...
  call kern_mul_r8<<<1,1>>>(A, B, res_r4, res_hi, res_lo)
  ...
  write(*,*) "=== fltflt multiplication (real(8) input): 1.1d0 * 1.1d0 ==="
  write(*,*)
