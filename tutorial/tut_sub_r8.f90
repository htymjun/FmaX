! tut_sub_r8.f90  —  fltflt subtraction with real(8) input
!
! Build:  cd tutorial && cmake -B build && cmake --build build -j
! Run:    cd build && ./tut_sub_r8
!
...
contains
  ...
  attributes(global) subroutine kern_sub_r8(a_in, b_in, res_r4, res_hi, res_lo)
  ...
  end subroutine kern_sub_r8

end module tut_sub_r8_kern
...
program tut_sub_r8
  ...
  call kern_sub_r8<<<1,1>>>(A, B, res_r4, res_hi, res_lo)
  ...
  write(*,*) "=== fltflt subtraction (real(8) input): (1 + 2^-25) - 1.0 ==="
  write(*,*)
