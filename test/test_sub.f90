! test_sub.f90  —  fltflt subtraction: accuracy + throughput
!
! Build:  cd test && cmake -B build && cmake --build build -j
! Run:    cd build && ./test_sub
!        (writes test/sub_results.csv; plot: cmake --build . --target plot_sub)
!
! Accuracy: NCASE scalar cases, real(8) inputs.
! Benchmark: chained x = x - b for real(4), real(8), fltflt.

module test_sub_kern
  use cudafor
  use fltflt
  implicit none
  private
  ...
  public :: kern_sub
  public :: kern_bench_sub_r4, kern_bench_sub_r8, kern_bench_sub_ff
...
contains
  ...
  ! ---- accuracy kernel ------------------------------------------------
  attributes(global) subroutine kern_sub(a_d, b_d, n, res_r4, res_hi, res_lo)
  ...
  attributes(global) subroutine kern_bench_sub_r4(n, niter, a_d, b_d, out_d)
    ...
    out_d(i) = x
  end subroutine kern_bench_sub_r4

  attributes(global) subroutine kern_bench_sub_r8(n, niter, a_d, b_d, out_d)
    integer, intent(in), value    :: n, niter
    real(8), intent(in),  device  :: a_d(n), b_d(n)
    ...
    out_d(i) = x
  end subroutine kern_bench_sub_r8

  attributes(global) subroutine kern_bench_sub_ff(n, niter, a_d, b_d, out_hi, out_lo)
    integer, intent(in), value    :: n, niter
    real(8), intent(in),  device  :: a_d(n), b_d(n)
  ...
  end subroutine kern_bench_sub_ff

end module test_sub_kern
...
program test_sub
  ...
  character(len=16), parameter :: CLABELS(N) = &
    [ character(len=16) :: "sub[cancel25]", "sub[cancel24]", "sub[large]", "sub[general]" ]
  ...
  csv_unit = 11
  open(unit=csv_unit, file='sub_results.csv', status='replace', action='write')
  write(csv_unit, '(A)') 'type,label,val1,val2'

  ! (1) a = 1+2^-25, b = 1: r4 loses the tiny bit entirely; result = 0
  a_r8(1) = 1.0d0 + 2.0d0**(-25);  b_r8(1) = 1.0d0
  ! (2) a = 1+2^-24, b = 1: r4 half-ULP rounding; also rounds to 0
  a_r8(2) = 1.0d0 + 2.0d0**(-24);  b_r8(2) = 1.0d0
  ! (3) large gap: 1e8 - (1e8-1) = 1; r4 loses the unit in ULP
  a_r8(3) = 1.0d8;                  b_r8(3) = 1.0d8 - 1.0d0
  ! (4) general
  ...
  a_d = a_r8;  b_d = b_r8
  call kern_sub<<<1, N>>>(a_d, b_d, N, res_r4, res_hi, res_lo)
  istat = cudaDeviceSynchronize()
  h_r4 = res_r4;  h_hi = res_hi;  h_lo = res_lo
  deallocate(a_d, b_d, res_r4, res_hi, res_lo)

  write(*,*) "=== test_sub: fltflt subtraction (real(8) inputs) ==="
  nfail = 0
  do i = 1, N
...
contains
  ...
  subroutine run_bench()
    ...
    ! real(4)
    call kern_bench_sub_r4<<<blk,thr>>>(NB, NI, ad, bd, out_r4)
    ...
    ! real(8)
    call kern_bench_sub_r8<<<blk,thr>>>(NB, NI, ad, bd, out_r8)
    is = cudaDeviceSynchronize()
    is = cudaEventRecord(t0, 0_8)
    call kern_bench_sub_r8<<<blk,thr>>>(NB, NI, ad, bd, out_r8)
    is = cudaEventRecord(t1, 0_8);  is = cudaEventSynchronize(t1)
    ...
    ! fltflt
    call kern_bench_sub_ff<<<blk,thr>>>(NB, NI, ad, bd, out_hi, out_lo)
    is = cudaDeviceSynchronize()
    is = cudaEventRecord(t0, 0_8)
    call kern_bench_sub_ff<<<blk,thr>>>(NB, NI, ad, bd, out_hi, out_lo)
    is = cudaEventRecord(t1, 0_8);  is = cudaEventSynchronize(t1)
    ...
    write(*,'("--- benchmark (N=2^20, NITER=100, chained sub) ---")')
