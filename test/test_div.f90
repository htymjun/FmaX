! test_div.f90  —  fltflt division: accuracy + throughput
!
! Build:  cd test && cmake -B build && cmake --build build -j
! Run:    cd build && ./test_div
!        (writes test/div_results.csv; plot: cmake --build . --target plot_div)
!
! Accuracy: NCASE scalar cases, real(8) inputs.
! Benchmark: chained x = x / b for real(4), real(8), fltflt.

module test_div_kern
  use cudafor
  use fltflt
  implicit none
  private
  ...
  public :: kern_div
  public :: kern_bench_div_r4, kern_bench_div_r8, kern_bench_div_ff
...
contains
  ...
  ! ---- accuracy kernel ------------------------------------------------
  attributes(global) subroutine kern_div(a_d, b_d, n, res_r4, res_hi, res_lo)
  ...
  attributes(global) subroutine kern_bench_div_r4(n, niter, a_d, b_d, out_d)
    ...
    out_d(i) = x
  end subroutine kern_bench_div_r4

  attributes(global) subroutine kern_bench_div_r8(n, niter, a_d, b_d, out_d)
    integer, intent(in), value    :: n, niter
    real(8), intent(in),  device  :: a_d(n), b_d(n)
    ...
    out_d(i) = x
  end subroutine kern_bench_div_r8

  attributes(global) subroutine kern_bench_div_ff(n, niter, a_d, b_d, out_hi, out_lo)
    integer, intent(in), value    :: n, niter
    real(8), intent(in),  device  :: a_d(n), b_d(n)
  ...
  end subroutine kern_bench_div_ff

end module test_div_kern
...
program test_div
  ...
  character(len=16), parameter :: CLABELS(N) = &
    [ character(len=16) :: "div[1/3]", "div[2/3]", "div[r8/3]", "div[1/7]" ]
  ...
  csv_unit = 11
  open(unit=csv_unit, file='div_results.csv', status='replace', action='write')
  write(csv_unit, '(A)') 'type,label,val1,val2'

  ! (1) 1/3 — classic non-representable quotient
  a_r8(1) = 1.0d0;                  b_r8(1) = 3.0d0
  ! (2) 2/3
  a_r8(2) = 2.0d0;                  b_r8(2) = 3.0d0
  ! (3) r8-precision numerator: (1 + 2^-20) / 3
  a_r8(3) = 1.0d0 + 2.0d0**(-20);  b_r8(3) = 3.0d0
  ! (4) 1/7 — more significant bits required than 1/3
  a_r8(4) = 1.0d0;                  b_r8(4) = 7.0d0
  ref = a_r8 / b_r8

  allocate(a_d(N), b_d(N), res_r4(N), res_hi(N), res_lo(N))
  a_d = a_r8;  b_d = b_r8
  call kern_div<<<1, N>>>(a_d, b_d, N, res_r4, res_hi, res_lo)
  istat = cudaDeviceSynchronize()
  h_r4 = res_r4;  h_hi = res_hi;  h_lo = res_lo
  deallocate(a_d, b_d, res_r4, res_hi, res_lo)

  write(*,*) "=== test_div: fltflt division (real(8) inputs) ==="
  nfail = 0
  do i = 1, N
...
contains
  ...
  subroutine run_bench()
    ...
    a_h = 1.23456789d0;  b_h = 1.00000013d0
    ...
    ! real(4)
    call kern_bench_div_r4<<<blk,thr>>>(NB, NI, ad, bd, out_r4)
    ...
    ! real(8)
    call kern_bench_div_r8<<<blk,thr>>>(NB, NI, ad, bd, out_r8)
    is = cudaDeviceSynchronize()
    is = cudaEventRecord(t0, 0_8)
    call kern_bench_div_r8<<<blk,thr>>>(NB, NI, ad, bd, out_r8)
    is = cudaEventRecord(t1, 0_8);  is = cudaEventSynchronize(t1)
    ...
    ! fltflt
    call kern_bench_div_ff<<<blk,thr>>>(NB, NI, ad, bd, out_hi, out_lo)
    is = cudaDeviceSynchronize()
    is = cudaEventRecord(t0, 0_8)
    call kern_bench_div_ff<<<blk,thr>>>(NB, NI, ad, bd, out_hi, out_lo)
    is = cudaEventRecord(t1, 0_8);  is = cudaEventSynchronize(t1)
    ...
    write(*,'("--- benchmark (N=2^20, NITER=100, chained div) ---")')
